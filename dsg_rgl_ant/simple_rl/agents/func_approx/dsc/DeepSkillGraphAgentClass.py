...
class DeepSkillGraphAgent(object):
    ...
    def dsg_run_loop(self, episodes, num_steps, starting_episode=0, max_interactions=200, nb_interaction_before_test=1000):
        successes = []
        evaluations_results = []
        evaluations_abscissas = []
        interaction_id = 0
        initial_state = None  # For verification
        for episode in range(starting_episode, starting_episode+episodes):
            initial_interaction_id = interaction_id
            ...
            step_number = 0
            print()
            print("  interaction id = ", interaction_id, sep="")
            print("  episode id = ", episode, sep="")
            print()
            ...
            if interaction_id > nb_interaction_before_test * len(evaluations_results) and ready_for_eval:
            ...
            print("")
            print("episode duration = ", interaction_id - initial_interaction_id)
            print("")

        return successes

    def evaluation(self, nb_tests=20):
        print("Evaluating dsg agent for a duration of ", nb_tests, " tests...", sep="", end="\r")
        ...
        print("Evaluating dsg agent for a duration of ", nb_tests, " tests... DONE", sep="")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mon_env", type=int, default=0)
    ...
    if args.env == "point-reacher":
        ...
        state_dim = overall_mdp.state_space_size()
        action_dim = overall_mdp.action_space_size()
    elif args.env == "hbrl-ant-maze":

        assert args.map in ["four_rooms", "hard_maze", "medium_maze", "join_rooms_medium"]

        if args.map == "four_rooms":
            max_interactions = 500000
        if args.map == "hard_maze":
            max_interactions = 700000
        if args.map == "medium_maze":
            max_interactions = 1200000
        if args.map == "join_rooms_medium":
            max_interactions = 1200000
