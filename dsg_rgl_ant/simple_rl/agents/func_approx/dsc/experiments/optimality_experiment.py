...
class OnlineModelBasedSkillChaining(object):
    def __init__(self, warmup_episodes, max_steps, gestation_period, use_vf,
        ...
        self.mdp = AntReacherMDP(False, goal_state=np.array((0, 0)))
    ...
    def is_chain_complete(self):
        return all([option.get_training_phase() == "initiation_done" for option in self.chain]) and self.mature_options[
            -1].is_init_true(self.mdp.init_state)
    ...
    def manage_chain_after_rollout(self, executed_option):
        ...
        if self.should_create_new_option():
            ...
            print(
                f"Creating {name}, parent {new_option.parent}, new_options = {self.new_options}, mature_options = {self.mature_options}")
    ...
    def create_model_based_option(self, name, parent=None):
        ...
        option = ModelBasedOption(parent=parent, mdp=self.mdp,
                                  ...
                                  name=name,
                                  path_to_model="mpc-ant-reacher.pkl",
...
def test_agent(exp, num_episodes, num_steps):
    ...
    def reset(position):
        print(f"Resetting test environment to {position}")
        exp.mdp.reset()
        exp.mdp.set_xy(position)
