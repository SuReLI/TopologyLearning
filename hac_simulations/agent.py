...
class Agent:
    def __init__(self,FLAGS, env, agent_params, simulation_id, outputs_directory):
    ...
    def check_goals(self,env):
        ...
        proj_subgoal[:2] = self.current_state[:2]
        proj_end_goal = env.project_state_to_end_goal(env.sim, self.current_state)
        proj_end_goal[:2] = self.current_state[:2]
        ...
        for i in range(self.FLAGS.layers):
            ...
            # If not highest layer, compare to subgoal thresholds
                ...
                assert (self.layers[-1].actor.action_space_bounds == self.layers[-2].actor.action_space_bounds).all()
                for goal in self.goal_array:
                    assert (- self.layers[-2].actor.action_space_bounds[:2] <= goal[:2]).all() and \
                           (goal[:2] <= self.layers[-2].actor.action_space_bounds[:2]).all()
    ...
    def initialize_networks(self):
        ...
        self.model_dir = self.outputs_directory + '/models_' + str(self.simulation_id)
    ...
    def train(self, env, episode_num, total_episodes):
        ...
        assert (- self.layers[-2].actor.action_space_bounds[:2] <= self.goal_array[self.FLAGS.layers - 1][:2]).all() and \
               (self.goal_array[self.FLAGS.layers - 1][:2] <= self.layers[-2].actor.action_space_bounds[:2]).all()
        ...
        goal_status, max_lay_achieved, nb_interactions = self.layers[self.FLAGS.layers-1].train(self, env, episode_num=episode_num)
        ...
        return goal_status[self.FLAGS.layers-1], nb_interactions
