...
def generate_xml(map_name: str) -> (dict, str):
    ...
    tree = etree.parse(current_directory + "/mujoco_files/ant_maze.xml")
    ...
    # Load maps data
    maze_array = np.array(importlib.import_module("maps." + map_name).maze_array)
    try:
        tile_size = np.array(importlib.import_module("maps." + map_name).tile_size)
    ...
    for wall_id, wall in enumerate(horizontal_walls):
        ...
        position = str(position_x * tile_size) + " " + str(position_y * tile_size) + " 0.5"
        wall_size = str((end_x - start_x + 1) * tile_size / 2) + " " + str(tile_size / 2) + " 0.5"
        node = E.body(
    ...
    for wall_id, wall in enumerate(vertical_walls):
        ...
        position = str(position_x * tile_size) + " " + str(position_y * tile_size) + " 0.5"
        wall_size = str(tile_size / 2) + " " + str((end_y - start_y + 1) * tile_size / 2) + " 0.5"
        node = E.body(
            E.geom(type="box", size=wall_size, contype="1", conaffinity="1", rgba="0.4 0.4 0.4 1"),
    ...
    xml_output_path = current_directory + "/mujoco_files/" + "rgl_" + map_name + ".xml"
    ...
    return maze_array, tile_size, xml_output_path
