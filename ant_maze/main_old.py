...
def run_simulation(agent, environment):
    ...
    while evaluation_id < local_settings.nb_evaluations_max:
        ...
        if episode_id % local_settings.nb_episodes_before_evaluation == 0:
...
def init():
    ...
    agent = TIPP(state_space=environment.observation_space, action_space=environment.action_space,
                 sub_goal_thresholds=environment.goal_thresholds[:2], random_exploration_duration=900,
                 max_steps_to_reach=40,
                 goal_reaching_agent=low_policy, verbose=False, edges_similarity_threshold=0.4,
                 nodes_similarity_threshold=0.5)
    """
