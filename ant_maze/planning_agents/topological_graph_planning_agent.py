...
class PlanningTopologyLearner(Agent):
    ...
    def __init__(self, **params):
        ...
        # ALGORITHM TYPE
        # re_usable_policy=True implies a pre-training to the given policy. This option implies that features describing
        # the task to do can be extracted from the state - goal couple to prevent over-fitting.
        self.re_usable_policy = params.get("re_usable_policy", True)
        self.explored_node_criteria = params.get("explored_node_criteria", "explorations")
        ...
        self.sub_goal_thresholds = params.get("sub_goal_thresholds", None)
        assert self.sub_goal_thresholds is not None, \
            "agent.sub_goal_threshold should be specified on agent's initialisation."
        ...
        self.name = params.get("name", "Topology Learner")
        ...
        self.state_to_sub_goal_filter = params.get("state_to_sub_goal_filter", None)
        if self.state_to_sub_goal_filter is None:
            self.state_to_sub_goal_filter = [1 if i < self.sub_goal_thresholds.shape[0] else 0
                                             for i in range(self.state_space.shape[0])]
        self.state_to_sub_goal_filter = np.array(self.state_to_sub_goal_filter).astype(np.bool)
        self.goal_reaching_agent = params.get("goal_reaching_agent")
        assert self.goal_reaching_agent is not None, "Goal reaching agent is required to navigate through the graph."
        ...
        self.higher_node_id = -1  # Index of the younger node. To know which id to give to a new node.
        ...
        self.nb_successes_on_edges = 0
        self.nb_failures_on_edges = 0
    ...
    def on_episode_start(self, *args):
        ...
        if self.mode == TopologyLearnerMode.LEARN_ENV:
            ...
            _, _, goal = args
            fill = state[len(goal):len(self.state_to_sub_goal_filter)]
            self.final_goal = np.concatenate((goal, fill))[self.state_to_sub_goal_filter]
    ...
    def action(self, state):
        ...
        return self.goal_reaching_agent.action(state, self.next_goal)
    ...
    def reached(self, state: np.ndarray, goal: np.ndarray = None) -> bool:
        if goal is None:
            goal = self.next_goal
        return (state[self.state_to_sub_goal_filter] - goal <= self.sub_goal_thresholds).all()
    ...
    def set_exploration_path(self, state):
        ...
        node_from = self.get_node_for_state(state)
        best_node = min(self.topology.nodes(data=True), key=lambda x: x[1][self.explored_node_criteria])[0]
    ...
    def get_node_for_state(self, state, data=False, reachable_only=False):
        ...
        :params data: return a tuple containing the node id and the node's data if true, and only the node id otherwise.
        :params reachable_only: If true, choose a node that is reachable from the node 0. Otherwise, it chose the
        closest node.
        """
        assert isinstance(state, np.ndarray)
        if state.shape[-1] == len(self.state_to_sub_goal_filter):
            state = state[self.state_to_sub_goal_filter]
        if not self.topology.nodes:
        ...
        for node_id, args in self.topology.nodes(data=True):
            if reachable_only:
                try:
                    ...
                    continue
            distance = self.get_states_distance(args["state"], state)
    ...
    def create_node(self, state, **params):
        ...
        attributes["state"] = state[self.state_to_sub_goal_filter]
    ...
    def copy(self):
        new_agent = type(self)(**self.init_params)  # shallow copy of whole object
        new_agent.topology = copy.deepcopy(self.topology)
        return new_agent
