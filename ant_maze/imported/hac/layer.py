...
class Layer:
    def __init__(self, layer_number, FLAGS, env, sess, agent_params):
        ...
        # Set time limit for each layer.  If agent uses only 1 layer, time limit is the max number of low-level actions
        # allowed in the episode (i.e, env.max_actions).
        if FLAGS.layers > 1:
    ...
    def choose_action(self, agent, env, subgoal_test):
        ...
        if agent.FLAGS.test or subgoal_test:
            ...
            return self.actor.get_action(self.current_state[np.newaxis], self.goal[np.newaxis])[0], \
                   "Policy", subgoal_test
            ...
            if np.random.random_sample() > agent.other_params["random_action_perc"]:
                # Choose noisy action
                action = self.add_noise(self.actor.get_action(self.current_state[np.newaxis], self.goal[np.newaxis])[0],
                                        env)
    ...
    def create_prelim_goal_replay_trans(self, hindsight_action, next_state, env, total_layers):
        ...
        # Transition will be in the form [old state, hindsight action, reward = None, next state, goal = None,
        # finished = None, next state projeted to subgoal/end goal space]
        ...
        """
        # Designer can create some additional goal replay transitions.  For instance, higher level transitions can be 
        replayed with the subgoal achieved in hindsight as the original goal.
        if self.layer_number > 0:
