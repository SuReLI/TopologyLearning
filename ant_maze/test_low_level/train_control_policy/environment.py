...
class Environment():
    ...
    def __init__(self, model_name, goal_space_train, goal_space_test, project_state_to_end_goal, end_goal_thresholds,
        ...
        if model_name == "pendulum.xml":
            ...
            self.state_dim = len(self.sim.data.qpos) + len(self.sim.data.qvel)  # State will include
            # (i) joint angles and
            # (ii) joint velocities
        ...
        # Convert sub_goal bounds to symmetric bounds and offset.
        # Need these to properly configure sub_goal actor networks
        ...
        if self.name == "ant_reacher.xml":
            while True:
                ...
                for i in range(len(self.sim.data.qpos)):
                    if i == 6:
                        self.sim.data.qpos[i] = random.random() * (math.pi * 2) - math.pi
                        # Sample random orientation for the beginning
                    else:
    ...
    def display_subgoals(self,subgoals):
        ...
        for i in range(1,min(len(subgoals),11)):
            if self.name == "pendulum.xml":
                ...
                # Visualize desired gripper position, which is elements 18-21 in sub_goal vector
