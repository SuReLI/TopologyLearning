...
def run_HAC(FLAGS,env,agent):
    ...
    max_goal_samples = 1000
    max_trajectories_samples = 10000
    total_episodes = 0
    ...
    for batch in range(NUM_BATCH):
        ...
        for episode in range(num_episodes):
            ...
            # Update pre_training information
            pre_train_information["stopwatch"].stop()
            pre_train_information["nb_interactions"] += len(trajectory)
            pre_train_information["nb_episodes"] += 1
            if len(pre_train_information["trajectories"]) > max_trajectories_samples:
                pre_train_information["trajectories"].pop(0)
            pre_train_information["trajectories"].append(trajectory)
            if success:
                if len(pre_train_information["reached_goals"]) > max_goal_samples:
                    pre_train_information["reached_goals"].pop(0)
