...
from old.src.agents.grid_world.graph_free.goal_conditioned_dqn_her_diff import DqnHerDiffAgent
...
class PlanningTopologyLearner(Agent):
    ...
    def __init__(self, **params):
        ...
        # the task to do can be extracted from the state - goal couple to prevent overfitting.
        ...
        default_goal_reaching_class = DqnHerDiffAgent if self.re_usable_policy else DQNHERAgent
        self.goal_reaching_agent_class = params.get("goal_reaching_agent_class", default_goal_reaching_class)
        ...
        self.goal_reaching_agent = self.goal_reaching_agent_class(state_space=self.state_space,
                                                                  action_space=self.action_space,
                                                                  device=self.device,
                                                                  state_to_goal_filter=self.state_to_goal_filter)
    ...
    def reached(self, state: np.ndarray, goal: np.ndarray = None) -> bool:
        if goal is None:
            goal = self.next_goal
        dist = np.linalg.norm(state[self.state_to_goal_filter] - goal, 2)
    ...
    def set_exploration_path(self, state):
        ...
        node_from = self.get_node_for_state(state)
        attribute = "explorations" if self.re_usable_policy else "reached"
        best_node = min(self.topology.nodes(data=True), key=lambda x: x[1][attribute])[0]
