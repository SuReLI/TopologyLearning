...
class DistributionalDQN(GoalConditionedAgent):
    """
    ...
    def __init__(self, **params):
        ...
        self.planning_horizon_length = params.get("planning_horizon_length", 20)  # AKA nb distribution outputs
        ...
        self.model = MLP(self.state_size * 2, self.layer_1_size, ReLU(), self.layer_2_size, ReLU(),
                         self.nb_actions * self.planning_horizon_length, Softmax(),
    ...
    def get_q_value(self, state, goal):
        with torch.no_grad():
            ...
            if len(state.shape) == 1:
                out = self.model(goal_conditioned_state)
                return - torch.max(out, dim=-1).indices
            elif len(state.shape) == 2:
                out = self.model(goal_conditioned_state)\
                    .reshape((state.shape[0], self.nb_actions, self.planning_horizon_length))
                return - torch.max(out, dim=-1).indices
            else:
    ...
    def action(self, state):
        ...
        if np.random.rand() < self.epsilon:  # Epsilon greedy
            ...
            with torch.no_grad():
                q_values = self.get_q_value(state, self.current_goal)
    ...
    def learn(self):
        for _ in range(self.nb_gradient_steps):
            ...
            if len(self.replay_buffer) > self.batch_size:
                ...
                q_prime = self.target_model(goal_conditioned_new_states)\
                    .reshape((self.batch_size, self.nb_actions, self.planning_horizon_length))
                q_prime = - torch.max(q_prime, dim=-1).indices
                q_prime = torch.argmax(q_prime, dim=-1)
                targets = rewards + self.gamma * (1 - dones) * q_prime
                targets = - torch.clip(torch.round(targets), -self.planning_horizon_length, 0).to(torch.long)

                q_s_a = self.model(goal_conditioned_states)\
                    .reshape((self.batch_size, self.nb_actions, self.planning_horizon_length))
                q_s_a = - torch.max(q_s_a, dim=-1).indices
                q_s_a = q_s_a.gather(1, actions.to(torch.long).unsqueeze(1))
                loss = self.criterion(q_s_a, targets.unsqueeze(1))
