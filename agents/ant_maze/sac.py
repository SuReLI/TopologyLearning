...
class SACAgent(Agent):
    def __init__(self, **params):
        ...
        self.critic = MLP(self.state_size + self.nb_actions, self.layer_1_size, ReLU(), self.layer_2_size, ReLU(),
    ...
    def learn(self):
        if len(self.replay_buffer) > self.batch_size:
            states, actions, rewards, new_states, done = self.replay_buffer.sample(self.batch_size)
            ...
            with torch.no_grad():
                next_actions, next_log_probs = \
                    self.sample_action(new_states, actor_network=self.target_actor)
                critic_input = torch.cat((new_states, next_actions), -1)
