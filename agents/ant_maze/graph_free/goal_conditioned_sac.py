...
class GoalConditionedSACAgent(GoalConditionedAgent):
    def __init__(self, **params):
        ...
        self.actor = MLP(self.state_size + self.goal_size, self.layer_1_size, ReLU(), self.layer_2_size, ReLU(),
                         2 * self.nb_actions, learning_rate=self.actor_lr, optimizer_class=optim.Adam,
                         device=self.device).float()
        self.target_actor = copy.deepcopy(self.actor)

        self.critic = MLP(self.state_size + self.goal_size + self.nb_actions, self.layer_1_size, ReLU(),
