...
class DiscreteSORB(DqnHerDiffAgent):
    ...
    def on_episode_start(self, *args):
        ...
        super().on_episode_start(state, self.next_goal())
    ...
    def on_action_stop(self, action, new_state, reward, done, learn=False):
        ...
        if not learn:  # We only learn at pretraining, we will not use graph there
            ...
            if self.sub_goals:
                ...
                dir = os.path.dirname(__file__) + "/output/episode_" + str(self.episode_id)
                create_dir(dir)
                ...
                for way_point in self.sub_goals:
                    self.environment.place_point(image, self.get_node_state(way_point), [0, 0, 255])
                save_image(image, dir, "test" + str(self.episode_time_step_id) + ".png")
    ...
    def get_distance_estimation(self, state, goal, normalised=True):
        ...
        """
        with torch.no_grad():
            q_value_estimation = torch.clip(torch.max(self.model(state - goal)), max=0).detach().item()
